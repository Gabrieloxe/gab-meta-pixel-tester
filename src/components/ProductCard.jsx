...
export const ProductCard = ({ product }) => {
  ...
  return (
    <Link to={product.link} className="card card-bordered bg-base-200 hover:shadow-2xl hover:-translate-y-1 transition-all">
      <figure className="relative h-48">
        <img src={product.image_link} alt={product.title} className="w-full h-full object-cover hover:scale-105 transition-transform" />
      ...
      <div className="card-body p-4 gap-2">
        <div className="badge badge-outline badge-sm">{product.product_type}</div>
        <h3 className="card-title text-sm leading-snug">{product.title}</h3>
        ...
        <div className="flex items-baseline gap-2 mt-auto">
          <span className="text-lg font-bold">${sellingPrice.toFixed(2)}</span>
          {discount > 0 && <span className="text-sm text-base-content/40 line-through">${product.price.toFixed(2)}</span>}
        </div>
